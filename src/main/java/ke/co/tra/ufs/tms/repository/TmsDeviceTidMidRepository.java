...
public interface TmsDeviceTidMidRepository extends CrudRepository<TmsDeviceTidsMids, Long> {
    ...
    @Query("SELECT COUNT(t.id) FROM TmsDeviceTidsMids t WHERE t.tid  IN (?1) and t.deviceIds NOT IN (?2)")
    Integer getTmsDeviceTidsMidsByDeviceIdsIn(Set<String> tid, Long deviceIds);

    @Query("SELECT t.id FROM TmsDeviceTidsMids t WHERE t.mid = ?1")
    Long getIdByMid(String mid);

    @Query("SELECT t.id FROM TmsDeviceTidsMids t WHERE t.mid = ?1 AND t.deviceIds = ?2")
    Long getIdByMidAndDeviceId(String mid, Long deviceIds);
}
