...
public class OnboardingResource {
    ...
    private final BankConfigFileService bankConfigFileService;
    ...
    public OnboardingResource(LoggerServiceLocal loggerService, DeviceService deviceService, BusinessUnitService businessUnitService,
                              ...
                              ParFileConfigService parFileConfigService, CustomerConfigFileService customerConfigFileService, BankConfigFileService bankConfigFileService, ParDeviceSelectedOptionsService parDeviceSelectedOptionsService,
        ...
        this.bankConfigFileService = bankConfigFileService;
...
//                tmsDeviceTidMids.setCurrencyIds(obj.getCurrencyIds());
                ...
                tmsDeviceTidMids.setUfsBankId(obj.getUfsBankId());
                tmsDeviceTidMids.setIsMainBank(obj.getIsMainBank());
                deviceService.saveDeviceTids(tmsDeviceTidMids);

                if(Objects.nonNull(obj.getCurrencies()) && obj.getCurrencies().size() > 0){
                    obj.getCurrencies().forEach(tidMidCurr ->{
                        TidMidCurrency tidMidCurrency = new TidMidCurrency();
                        tidMidCurrency.setCurrencyIds(tidMidCurr);
                        tidMidCurrency.setTidMidIds(tmsDeviceTidMids.getId());
                        deviceService.saveDeviceTidsMidsCurrency(tidMidCurrency);
                    });
                }
            });
        }
    ...
    private void transferAndCopyFiles(DevicesWrapper onboardWrapper, TmsDevice tmsDevice, TmsDeviceTask deviceTask) throws IOException {
        ...
        try {
            ...
            if (onboardWrapper.getMasterProfileId() != null) {
                ...
                bankConfigFileService.generateBankFile(tmsDevice.getDeviceId(), rootPath);
...
//                            parFileConfigService.generateGlobalConfigFileAsync(config.getConfigProfile(),    new GlobalConfigFileRequest(addTaskWrapper.getModelId(), config.getId()), rootPath);
            ...
            bankConfigFileService.generateBankFile(addTaskWrapper.getDeviceId(), rootPath);
            loggerService.logCreate("Saving new App Files", SharedMethods.getEntityName(TmsDevice.class), tmsDevice.getDeviceId(), AppConstants.STATUS_COMPLETED);
