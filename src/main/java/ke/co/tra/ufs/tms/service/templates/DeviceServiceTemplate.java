...
public class DeviceServiceTemplate implements DeviceService {
    ...
    public DeviceServiceTemplate(MakeRepository makeRepo, ModelRepository modelRepo, FTPLogsRepository fTPLogsRepository,
                                 DeviceTypeRepository typeRepo, WhitelistRepository whitelistRepo, UfsBankRepository bankRepository, DeviceSimcardRepository deviceSimcardRepository,
            ...
            , ParBinConfigRepository parBinConfigRepository, TmsDeviceTidMidRepository tmsDeviceTidRepository, TmsDeviceSimcardRepository tmsDeviceSimcardRepository, UfsCurrencyRepository ufsCurrencyRepository, BusinessUnitItemRepository businessUnitItemRepository, UfsTerminalHistoryService terminalHistoryService,
                                 UfsCustomerOutletRepository customerOutletRepository, UfsContactPersonRepository contactPersonRepository, DeviceTypeRepository deviceTypeRepository, MakeRepository makeRepository, UfsCustomerRepository customerRepository, TidMidCurrencyRepository tidMidCurrencyRepository) {
    ...
    public void processWhitelistUpload(TmsWhitelistBatch batch, SysConfigService configService, SharedMethods sharedMethods, LoggerServiceLocal loggerService, byte[] file, String ipAddress, String userAgent, Long userId, WhitelistWrapper payload) {
        try {
            ...
            for (WhitelistDetails entity : entities) {
                ...
                //check if bank code doesn't exist
                UfsBanks bank = this.bankRepository.findByBankCodeAndIntrash(entity.getBankCode(), AppConstants.NO);
                if (bank == null) {
                    loggerService.logCreate("Whitelisting a device (Serial number:  "
                                    + entity.getSerialNo() + ") failed. Bank code does not exists",
                            SharedMethods.getEntityName(TmsWhitelist.class), null,
                            AppConstants.STATUS_FAILED, userId, ipAddress, userAgent);
                    failed++;
                    continue;
                ...
                device = new TmsWhitelist(entity.getSerialNo(),
                        ...
                        AppConstants.ACTION_STATUS_UNCONFIRMED, AppConstants.NO,bank.getId());
    ...
    public Page<TmsWhitelist> getWhitelist(String actionStatus, String modelId,
                                           Date from, Date to, String needle, String serialNo, String assignStr,String ufsBankId, Pageable pg) {
        return this.whitelistRepo.findAll(actionStatus, modelId, from, to, needle.toLowerCase(), AppConstants.NO, serialNo, assignStr,ufsBankId, pg);
    }

    @Override
    public Page<TmsWhitelist> getDeletedWhitelistDevices(String actionStatus, String modelId, Date from, Date to, String needle, String serialNo, String assignStr,String ufsBankId, Pageable pg) {
        return this.whitelistRepo.findAll(actionStatus, modelId, from, to, needle.toLowerCase(), AppConstants.YES, serialNo, assignStr,ufsBankId, pg);
...
//        scheduler.setProductId(deviceId.getEstateId().getUnitId().getProductId());
    ...
    @Override
    public TidMidCurrency saveDeviceTidsMidsCurrency(TidMidCurrency tidMidCurrency) {
        return tidMidCurrencyRepository.save(tidMidCurrency);
    }
...
//            List<TmsDeviceTidsMids> midQuery1 = tmsDeviceTidRepository.findAllByMidAndCurrencyIdsIsNot(tm.getMid(), tm.getCurrencyIds());
            int counter = 0;
            Long tidMidIds = tmsDeviceTidRepository.getIdByMid(tm.getMid());
            if(Objects.nonNull(tm.getCurrencies()) && tm.getCurrencies().size()>0){
                for(BigDecimal curr: tm.getCurrencies()){
                    List<TidMidCurrency> tidMidQuery = tidMidCurrencyRepository.findAllByMidAndCurrencyIdsIsNot(tidMidIds,curr);
                    counter = tidMidQuery.size();
                }
            }
            if(counter>0){
                return true;
...
//            List<TmsDeviceTidsMids> midQuery = tmsDeviceTidRepository.findAllByMidAndCurrencyIdsIsNotAndDeviceIds(tm.getMid(), tm.getCurrencyIds(), deviceId.longValue());
            int counter = 0;
            Long tidMidIds = tmsDeviceTidRepository.getIdByMidAndDeviceId(tm.getMid(), deviceId.longValue());
            if(Objects.nonNull(tm.getCurrencies()) && tm.getCurrencies().size()>0){
                for(BigDecimal curr: tm.getCurrencies()){
                    List<TidMidCurrency> tidMidQuery = tidMidCurrencyRepository.findAllByMidAndCurrencyIdsIsNot(tidMidIds,curr);
                    counter = tidMidQuery.size();
