...
import ke.tracom.ufs.services.template.BeanUtil;
...
public class UfsOrganizationUnits implements Serializable {
    ...
    public UfsOrganizationHierarchy getNextLevelTenant() {
        try {
            OrganizationService orgService = BeanUtil.getBean(OrganizationService.class);
            Optional<UfsOrganizationHierarchy> hie = orgService.findByLevelNo(getLevelId().getLevelNo().add(new BigDecimal(1)));
            if (hie.isPresent()) {
                return hie.get();
            }
        } catch (Exception e) {
